...
export function buildJudgePrompt({ slotsWithResponses, answerKeyTrimmed, judgeWebContext, promptText, judgeFeedback, judgeInstructions }) {
  ...
  const parts = [
    ...
    '',
    answerKeyTrimmed
      ? 'BASIS FOR SCORING: An ANSWER KEY is provided below. Judge whether each model\'s response is SUBSTANTIVELY CORRECT—meaning the factual content matches the answer key. IMPORTANT: If a model\'s response contains a line starting with "Final Answer:", use THAT line as the model\'s answer—ignore any verbose reasoning above it. If there is no "Final Answer:" line, evaluate the full response. Ignore trivial differences like capitalization, punctuation, phrasing, formatting, or word order. "ampere" and "Ampere" are the same answer. Focus ONLY on whether the model gave the right factual answer. A correct answer = 8-10. A partially correct answer = 4-7. A wrong answer = 1-3. No response = 0.'
      : 'BASIS FOR SCORING: No answer key was provided. Use the WEB SEARCH section below (if present) to fact-check, or use your own knowledge to evaluate correctness. IMPORTANT: If a model\'s response contains a line starting with "Final Answer:", use THAT line as the model\'s answer—ignore any verbose reasoning above it. Focus on factual accuracy. Ignore trivial formatting differences.',
    '',
